###############################################################################################################
#   Revision LScene:
#   - Create test data based on a specific Landsat scene path/row. Historical climate data is not generated,
...
# If a geometry is given through below box, the data in the area defined by this geometry will be extracted and
# given Landsat row/path is ignored. Otherwise given Landsat row/path will determine the area to be extracted.
box = ee.Geometry.Polygon([[-76.53527523327742, 42.48056760133223],
                           [-76.53527523327742, 42.41013764906172],
                           [-76.41614223767195, 42.41013764906172],
                           [-76.41614223767195, 42.48056760133223]])
#box = None
row_no = 39
path_no = 35
prefix = 'LSS'                    # This prefix will be the starting string in output file name
GCbucketName = 'shahriarsh_temp'  # Google Cloud bucket to save output result
extract_periods = [('2014-01-01','2014-03-01')]
addSpectralIndices = True  # if true, below SPIs are added to the output data.
SPIs2Create = ['BSI','DD','ENDISI','MNDWI','MSAVI2','NDVI','NLI','VSDI']
SPIs2Add = ['ENDISI']
maskCloud = True
addGLCM = True  # if true, below GLCM_features are added to the output data.
# NOTE: only one base, neighborhood, and qlevel is allowed per entry but metrics can be multiple.
GLCM_features = [
...
addTerrainBands = True                  # if true, will add terrain data (from SRTM)
...
patch_size = 100                        # TFRecord Data will be exported out of GEE with ths patch size
...
for period in extract_periods:
    print('Processing Landsat Path{} / Row{} for period {} ...'.format(path_no, row_no, period))
    start = period[0]
    end = period[1]
    if not box:
        wholeScene = True
        inputMap = Landsat7SR.filter(ee.Filter.eq('WRS_PATH', path_no)).filter(ee.Filter.eq('WRS_ROW', row_no)).first()
        box = inputMap.geometry()
    else:
        wholeScene = False
        inputMap = Landsat7SR.filterBounds(box).first()
    ...
    if wholeScene:
        LandsatCol1 = Landsat5SR.filterDate(start, end).filter(ee.Filter.eq('WRS_PATH', path_no)).\
            filter(ee.Filter.eq('WRS_ROW', row_no)).select(ETM_selected_bands,processed_bands)
        LandsatCol2 = Landsat7SR.filterDate(start, end).filter(ee.Filter.eq('WRS_PATH', path_no)).\
            filter(ee.Filter.eq('WRS_ROW', row_no)).select(ETM_selected_bands,processed_bands)
        LandsatCol3 = Landsat8SR.filterDate(start, end).filter(ee.Filter.eq('WRS_PATH', path_no)).\
            filter(ee.Filter.eq('WRS_ROW', row_no)).select(OLI_selected_bands,processed_bands)
    else:
    ...
    if wholeScene:
        AssetName = prefix + '_' + str(path_no).zfill(3) + str(row_no).zfill(3) + '_' + start + '_' + end + '_' + str(NBands) + 'band_AI'
    else:
        AssetName = prefix + '_' + start + '_' + end + '_' + str(NBands) + 'band_AI'
    ...
    task = ee.batch.Export.image.toCloudStorage(
        ...
        formatOptions={
            ...
            'collapseBands': True, # The output combined band will have the name of the first band, i.e. DOY
            'compressed': True
        }
    )
    task.start()
